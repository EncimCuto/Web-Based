...
$query = "SELECT TimeDivert FROM `readsensors` WHERE DATE(waktu) >= '$tanggal' ORDER BY id DESC LIMIT 1";
$result = mysqli_query($koneksi, $query);

$data = array();
// Ambil nilai level feed water dari hasil query
if ($row = mysqli_fetch_assoc($result)) {
    $data['TD1'] = $row['TimeDivert']; // Data level feed water
} else {
    $data['TD1'] = 0; // Jika tidak ada data, set default ke 0
}
