...
try {
    // Koneksi ke database
    $pdo = new PDO('mysql:host=localhost;dbname=boilerdb', 'root', '');
    ...
    if ($interval === 'jam') {
        ...
        $query = "SELECT 
                      ...
                      SUBSTRING_INDEX(GROUP_CONCAT(LHGuiloutine ORDER BY waktu), ',', 1) AS LHGuiloutine,
                      SUBSTRING_INDEX(GROUP_CONCAT(RHTemp ORDER BY waktu), ',', 1) AS RHTemp,
                      SUBSTRING_INDEX(GROUP_CONCAT(LHTemp ORDER BY waktu), ',', 1) AS LHTemp,
                      SUBSTRING_INDEX(GROUP_CONCAT(PVSteam ORDER BY waktu), ',', 1) AS PVSteam,
                      SUBSTRING_INDEX(GROUP_CONCAT(LHFDFan ORDER BY waktu), ',', 1) AS LHFDFan,
                      SUBSTRING_INDEX(GROUP_CONCAT(RHFDFan ORDER BY waktu), ',', 1) AS RHFDFan,
                      SUBSTRING_INDEX(GROUP_CONCAT(LHStoker ORDER BY waktu), ',', 1) AS LHStoker,
                      SUBSTRING_INDEX(GROUP_CONCAT(RHStoker ORDER BY waktu), ',', 1) AS RHStoker,
                      SUBSTRING_INDEX(GROUP_CONCAT(WaterPump1 ORDER BY waktu), ',', 1) AS WaterPump1,
                      SUBSTRING_INDEX(GROUP_CONCAT(WaterPump2 ORDER BY waktu), ',', 1) AS WaterPump2,
                      SUBSTRING_INDEX(GROUP_CONCAT(InletWaterFlow ORDER BY waktu), ',', 1) AS InletWaterFlow,
                      SUBSTRING_INDEX(GROUP_CONCAT(OutletSteamFlow ORDER BY waktu), ',', 1) AS OutletSteamFlow,
                      SUBSTRING_INDEX(GROUP_CONCAT(SuhuFeedTank ORDER BY waktu), ',', 1) AS SuhuFeedTank
                  FROM readsensors 
    ...
    // Menyimpan file Excel dan mengunduh langsung
    ...
    $fileName = 'Data Boiler ' . $tanggal_awal_formatted . ' hingga ' . $tanggal_akhir_formatted . '.xlsx';

    // Mengunduh file langsung tanpa menyimpan di server
