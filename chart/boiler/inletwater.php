...
<script>
    ...
    function kirimRequest(phpFile) {
        ...
        fetch(`./API/${phpFile}?tanggal-awal=${encodeURIComponent(tanggalAwal)}&tanggal-akhir=${encodeURIComponent(tanggalAkhir)}&data=${selectedData}`)
            ...
            .then(data => {
                ...
                const labels = (data.InletWaterFlow || []).map(item => item.waktu);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const data11Checkbox = document.getElementById('data11');
        if (data11Checkbox) {
            data11Checkbox.checked = true;
        }
    });
