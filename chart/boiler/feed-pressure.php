...
<script>
    ...
    function kirimRequest(phpFile) {
        ...
        fetch(`./API/${phpFile}?tanggal-awal=${encodeURIComponent(tanggalAwal)}&tanggal-akhir=${encodeURIComponent(tanggalAkhir)}&data=${selectedData}`)
            ...
            .then(data => {
                ...
                const labels = (data.FeedPressure || []).map(item => item.waktu);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const feedPressureCheckbox = document.getElementById('feedPressure');
        if (feedPressureCheckbox) {
            feedPressureCheckbox.checked = true;
        }
    });
