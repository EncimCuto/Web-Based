...
<script>
    ...
    function kirimRequest(phpFile) {
        ...
        fetch(`./API/${phpFile}?tanggal-awal=${encodeURIComponent(tanggalAwal)}&tanggal-akhir=${encodeURIComponent(tanggalAkhir)}&data=${selectedData}`)
            ...
            .then(data => {
                ...
                const labels = (data.OutletSteamFlow || []).map(item => item.waktu);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const data12Checkbox = document.getElementById('data12');
        if (data12Checkbox) {
            data12Checkbox.checked = true;
        }
    });
