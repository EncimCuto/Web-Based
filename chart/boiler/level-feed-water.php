...
<script>
    ...
    function kirimRequest(phpFile) {
        ...
        fetch(`./API/${phpFile}?tanggal-awal=${encodeURIComponent(tanggalAwal)}&tanggal-akhir=${encodeURIComponent(tanggalAkhir)}&data=${selectedData}`)
            ...
            .then(data => {
                ...
                const labels = (data.LevelFeedWater || []).map(item => item.waktu);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const feedWaterCheckbox = document.getElementById('feedWater');
        if (feedWaterCheckbox) {
            feedWaterCheckbox.checked = true;
        }
    });
