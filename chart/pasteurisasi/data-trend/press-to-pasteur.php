...
<script>
    ...
    function kirimRequest(phpFile) {
        ...
        const selectedData = Array.from(document.querySelectorAll('input[name="data"]:checked'))
            ...
            .then(data => { // Ganti $data dengan data
                ...
                const labels = (data.PressToPasteur || []).map(item => item.waktu);
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const data16Checkbox = document.getElementById('data16');
        if (data16Checkbox) {
            data16Checkbox.checked = true;
        }
    });
