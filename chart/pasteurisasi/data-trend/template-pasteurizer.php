...
<div class="info">
    <div class="dropdown">
        ...
        <div class="dropdown-content">
            <a href="./flowrate.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Flowrate</a>
            <a href="./preheating.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Suhu Preheating</a>
            <a href="./cooling.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Suhu Cooling</a>
            <a href="./heating.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Suhu Heating</a>
            <a href="./holding.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Suhu Holding</a>
            <a href="./precooling.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Suhu Precooling</a>
            <a href="./pumpBT2.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Speed Pump BT 2</a>
            <a href="./levelBT2.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Level BT 2</a>
            <a href="./pressureBT2.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Pressure BT 2</a>
            <a href="./pcv1.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">PCV 1</a>
            <a href="./divert.php?token=<?php echo htmlspecialchars($_SESSION['token']); ?>">Time Divert</a>
        </div>
    </div>
...
<script>
    ...
    function exportData(interval) {
        ...
        const selectedCheckboxes = Array.from(document.querySelectorAll('input[name="data"]:checked'));
        const selectedData = selectedCheckboxes.map(checkbox => checkbox.value).join(',');

        // Gabungkan nama data yang dipilih untuk deskripsi
        const deskripsi = selectedCheckboxes.map(checkbox => {
            const label = checkbox.nextElementSibling; // Ambil label setelah checkbox
            return label ? label.textContent.trim() : ''; // Pastikan label ada sebelum mengakses textContent
        }).filter(Boolean).join(', '); // Menggabungkan dengan koma dan spasi, filter untuk menghindari string kosong

        // Format deskripsi untuk lebih rapi
        const formattedDeskripsi = deskripsi.replace(/,/g, ', ').trim(); // Pastikan spasi setelah koma
